...
%   Copyright 2020, SJTU.
%-----------------------------------------------------------------%
%Specturm of L for an infinite helical duct with k=2/3,h=1,k=0.95kc
%Main-Function
%   ref:https://github.com/Jiaqi-knight/NonlinearWaveguideCoding
%   Email:Jiaqi_Wang@sjtu.edu.cn
%   Copyright 2020, SJTU.
%-----------------------------------------------------------------%
%MainFunction for boundary, only s(end) of the function is solved!
clc
clear
close all
...
[Base_end]=BaseJ(Geo.m,Geo.n,Geo.h(end));


Psi.ab_r=X(Base_end,Geo,2,'ab','r').*Theta(Geo,2,'ab');
Psi.a_pr_b_r =X(Base_end,Geo,2,'a_pr_b','r').*Theta(Geo,2,'ab');
Psi.ab_pt_ab =X(Base_end,Geo,2,'ab','1').*Theta(Geo,2,'pt_ab');
Psi.ab_r2_cos =X(Base_end,Geo,2,'ab','r2').*Theta(Geo,2,'ab_cos');
Psi.ps_ab_r_1 =X(Base_end,Geo,2,'ps_ab','r').*Theta(Geo,2,'ab');
Psi.ps_ab_r_2 =X(Base_end,Geo,2,'ab','r').*Theta(Geo,2,'ps_ab');
Psi.a_ps_b_r_1 =X(Base_end,Geo,2,'a_ps_b','r').*Theta(Geo,2,'ab');
Psi.a_ps_b_r_2 =X(Base_end,Geo,2,'ab','r').*Theta(Geo,2,'a_ps_b');
Psi.pr_ab_r=X(Base_end,Geo,2,'pr_ab','r').*Theta(Geo,2,'ab');
Psi.pr_ab_r2_cos=X(Base_end,Geo,2,'pr_ab','r2').*Theta(Geo,2,'ab_cos');
Psi.pt_ab=X(Base_end,Geo,2,'ab','1').*Theta(Geo,2,'pt_ab');
Psi.pt_ab_r_cos=X(Base_end,Geo,2,'ab','r').*Theta(Geo,2,'pt_ab_cos');
Psi.ab_s1= specialFun(Geo.s,Geo.h,Geo.kappa,Geo.m,Geo.n,Base_end.Cmn1,Base_end.jmn_pm,'hh`^2/[1-\kappa*h*cos\psi]');
Psi.ab_s2= specialFun(Geo.s,Geo.h,Geo.kappa,Geo.m,Geo.n,Base_end.Cmn1,Base_end.jmn_pm,'h(1-\kappa*h*cos\psi)');
Psi.ab=X(Base_end,Geo,2,'ab','1').*Theta(Geo,2,'ab');
Psi.ab_r_cos=X(Base_end,Geo,2,'ab','r').*Theta(Geo,2,'ab_cos');
Psi.pt_ab_cos=X(Base_end,Geo,2,'ab','1').*Theta(Geo,2,'pt_ab_cos');

Psi.abc=X(Base_end,Geo,3,'ab','1').*Theta(Geo,3,'ab');
Psi.abc_r=X(Base_end,Geo,3,'ab','r').*Theta(Geo,3,'ab');
Psi.abc_r2_cos=X(Base_end,Geo,3,'ab','r2').*Theta(Geo,3,'ab_cos');
Psi.abc_r_cos= X(Base_end,Geo,3,'ab','r').*Theta(Geo,3,'ab_cos');
Psi.abc_r_sin=X(Base_end,Geo,3,'ab','r').*Theta(Geo,3,'ab_sin');
Psi.pr_abc_r=X(Base_end,Geo,3,'pr_ab','r').*Theta(Geo,3,'ab');
Psi.pr_abc_r2_cos=X(Base_end,Geo,3,'pr_ab','r2').*Theta(Geo,3,'ab_cos');
Psi.pt_abc=X(Base_end,Geo,3,'ab','1').*Theta(Geo,3,'pt_ab');
Psi.pt_abc_r_cos=X(Base_end,Geo,3,'ab','r').*Theta(Geo,3,'pt_ab_cos');
Psi.abc=X(Base_end,Geo,3,'ab','1').*Theta(Geo,3,'ab');
Psi.abc_r_cos=X(Base_end,Geo,3,'ab','r').*Theta(Geo,3,'ab_cos');
Psi.abc_r_sin=X(Base_end,Geo,3,'ab','r').*Theta(Geo,3,'ab_sin');
Psi.ps_abc_ps_r=X(Base_end,Geo,3,'ps(ab)','r').*Theta(Geo,3,'ab');  %(Jiaqi-153)
Psi.ps_abc_r=X(Base_end,Geo,3,'ps_ab','r').*Theta(Geo,3,'ab');
Psi.ps_abc=X(Base_end,Geo,3,'ps_ab','1').*Theta(Geo,3,'ab');
toc
tic
