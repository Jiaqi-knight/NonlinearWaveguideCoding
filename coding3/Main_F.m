...
Fun_b.G=-bsxfun(@times,Psi.ps_ab_r_1,reshape(ones(size(b)),1,1,1,length(b)))...
...
Fun3_ab.I3=bsxfun(@times,Psi.abc_r,reshape(bsxfun(@times,ones(size(Geo.s.'))*ones(size(b)),reshape(ones(size(a)),1,1,[])),1,1,1,length(Geo.s),length(b),length(a)));
...
Fun3_ab.A = -Fun3_ab.A_1...
            -multiprod(permute(multiprod(Fun3_ab.A_2,Fun3_ab.N_inv,[2,3]),[2,3,1,4,5,6]),(-multiprod(Fun3_ab.I3,Fun3_ab.A_2_1,[2,3])-Fun3_ab.A_2_2),[1,2])...
            -multiprod(permute(multiprod(Fun3_ab.A_3,Fun3_ab.N_inv,[2,3]),[2,3,1,4,5,6]),(multiprod(Fun3_ab.I3,Fun3_ab.A_3_1_2,[2,3])+Fun3_ab.A_3_2),[1,2]);%(James-3.35e)
...
Fun3_ab.B_5_1=multiprod(multiprod(Fun3_ab.I3,Fun3_a_b.M,[1,2]),Fun3_b.V,[2,3]);
...
Fun3_ab.B_6_1=multiprod(multiprod(Fun3_ab.I3,Fun3_a_b.M,[1,2]),Fun3_b.W,[2,3]);
...
Fun3_ab.C_1=multiprod(multiprod(Fun3_ab.I3,Fun3_a_b.I,[1,2]),Fun3_b.M,[2,3]);
Fun3_ab.C_2=multiprod(multiprod(Fun3_ab.I3,Fun3_a_b.M,[1,2]),Fun3_b.I,[2,3]);
...
for ka=1:length(a)
    for kb=1:length(b)
        Fun2_a.B_3(:,:,:,:,kb,ka)=multiprod(multiprod(Fun2_a.B_2(:,:,:,:,kb,ka),...
...
for ka=1:length(a)
    for kb=1:length(b)
        Fun2_a.B_4(:,:,:,:,kb,ka)=multiprod(multiprod(Fun2_a.B_2(:,:,:,:,kb,ka),...
...
for ka=1:length(a)
    for kb=1:length(b)
        Fun2_a.B_5_1(:,:,:,:,kb,ka)=multiprod(multiprod(Fun2_a.B_2(:,:,:,:,kb,ka),...
            permute(1/(sqrt(-1)*k*(a(ka)-b(kb)))*X(Base,Geo,2,'ab','r').*Theta(Geo,2,'pt_ab'),[1,2,4,3]),[1,2]),...
            permute(1/(sqrt(-1)*k*(b(kb)))*X(Base,Geo,2,'ab','r').*Theta(Geo,2,'pt_ab'),[4,1,2,3]),[2,3]);
    end
end
...
Fun2_a.M=-Fun2_a.N-mtimesx(Fun2_a.M_2_1,Fun2_a.V)-mtimesx(Fun2_a.M_3_1,Fun2_a.W);%(James-3.35a)

Fun2_a.M_2_1=(bsxfun(@times,X(Base,Geo,2,'pr_ab','r').*Theta(Geo,2,'ab'),reshape(ones(size(a)),1,1,1,length(a)))...
    -bsxfun(@times,X(Base,Geo,2,'pr_ab','r2').*Theta(Geo,2,'ab_cos'),reshape(Geo.kappa.'.*ones(size(a)),1,1,length(Geo.kappa),length(a))));
Fun2_a.M_3_1=(bsxfun(@times,X(Base,Geo,2,'ab','1').*Theta(Geo,2,'pt_ab'),reshape(ones(size(a)),1,1,1,length(a)))...
    -bsxfun(@times,X(Base,Geo,2,'ab','r').*Theta(Geo,2,'pt_ab_cos'),reshape(Geo.kappa.'.*ones(size(a)),1,1,length(Geo.kappa),length(a))));

Fun2_a.N=bsxfun(@times,X(Base,Geo,2,'ab','r').*Theta(Geo,2,'ab'),reshape((sqrt(-1)*k*a),1,1,1,length(a)))...
...
1+[1;2]
1/(sqrt(-1)*k*(a(2)-b(6)))
Fun2_a.B_3=Fun2_a.B_2*Fun2_a.V*Fun2_a.V
size(Fun2_a.B_2)
size(Fun2_a.V)

size(Fun2_a.A_2)

multiprod(Fun2_a.I3,Fun2_a.A_2_1,[2,3],[2,3]);
...
mtimesx(permute(T,[2,1,3,4]),permute(Fun2_a.I3,[3,2,1,4,5,6]))
% permute(Fun_a.I3,)
size(T)
% mtimesx(,)
% length(size(-Fun_a.I3))
%
% size(Fun_a.A_2)
% size(Fun_a.N_inv)
%Fun_a.A_2=
