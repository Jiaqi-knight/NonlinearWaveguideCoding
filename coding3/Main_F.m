...
G=-bsxfun(@times,X2{3,2}.*Theta(m,n,2,'ab'),reshape((sqrt(-1)*ones(size(a)).'*ones(size(kappa))),1,1,length(a),length(kappa)))... %(Jiaqi-147)
    -bsxfun(@times,X2{1,2}.*Theta(m,n,2,'ps_ab'),reshape((sqrt(-1)*ones(size(a)).'*ones(size(kappa))),1,1,length(a),length(kappa)));
H=
% A
% B
% C
