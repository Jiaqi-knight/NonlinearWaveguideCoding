...
+\underline{term \mathcal{X}2:} \kappa \Psi_{\alpha\beta\gamma}[1-\kappa rcos\phi]W_\beta^{a-b}W_\gamma^b-\kappa \Psi_{\alpha\beta\gamma}[r cos\phi]U_\beta^{a-b}U_\gamma^b\\
\end{aligned}
\end{equation}
...
\subsection{$V_\alpha^a $ \& $W_\alpha^a $}
...
\begin{equation}
\begin{aligned}
V_\alpha^a ={1\over {i a \kappa}}[\int_0^{2\pi}[r\psi_\alpha\psi_\beta]_0^h d\theta P_\beta^a-\Psi_{\alpha\beta}- \Psi_{[\alpha]\beta}[r]]P_\beta^a=\textbf{V}_{\alpha\beta}^a P_\beta^a
\end{aligned}
\end{equation}

\begin{equation}
\begin{aligned}
W_\alpha^a =-{1\over {i a \kappa}}\Psi_{(\alpha)\beta}P_\beta^a=\textbf{W}_{\alpha\beta}^a P_\beta^a
\end{aligned}
\end{equation}

\subsection{ ${d\over ds}V_\alpha^a$  \& ${d\over ds}W_\alpha^a$}
...
\begin{equation}
\begin{aligned}
{d \over ds} V_\alpha^a=\int_0^{2\pi}{{h'}^2\over {1-\kappa h cos\phi}}[r \psi_\beta \psi_\alpha ]_0^h d\theta +\Psi_{\{\alpha\}\beta}[r]V_\beta^a\\
+\int_0^{2\pi}[ r (1-\kappa r cos\phi) \psi_\beta \psi_\alpha]_0^h d\theta U_\beta^a
-\Psi_{[\alpha]\beta}[r(1-\kappa r cos\phi)]U_\beta^a
-\Psi_{\alpha\beta}[(1-\kappa r cos\phi)]U_\beta^a\\
\end{aligned}
...
\begin{equation}
\begin{aligned}
{d \over ds} V_\alpha^a=\int_0^{2\pi}{dh(s)\over ds} [ \psi_\beta r  \psi_\alpha ]_0^h d\theta W_\beta^a+\Psi_{\{\alpha\}\beta}[r]W_\beta^a
-\Psi_{(\alpha)\beta}[1-\kappa r cos\phi]U_\beta^a 
\end{aligned}
\end{equation}
...
\begin{equation}
\begin{aligned}
\Psi_{[\alpha]\beta}[ r(1-\kappa r cos(\phi))]\uuline{V_\beta^a}:=\Psi_{[\alpha]\delta}[ r(1-\kappa r cos(\phi))] \textbf{V}_{\delta\beta}^a P_\beta^a\\
\Psi_{(\alpha)\beta}[ (1-\kappa r cos(\phi))]\uuline{W_\beta^a}:=\Psi_{(\alpha)\delta}[ (1-\kappa r cos(\phi))] \textbf{W}_{\delta\beta}^a P_\beta^a\\
\end{aligned}
\end{equation}
...
+\underline{term \mathcal{D}1:} -{{\partial }\over {\partial s}}(\Psi_{\alpha\beta\gamma}[r])U_\beta^{a-b}U_\gamma^{b}
-\Psi_{\alpha\beta\gamma}[r]({{d U_\beta^{a-b} \over {d s}}U_\gamma^{b}}+{dU_\gamma^{b}\over ds}U_\beta^{a-b})\\
+\underline{term \mathcal{X}1:} \kappa \Psi_{\alpha\beta\gamma}[rcos\phi]U_\beta^{a-b}\uuline{V_\gamma^b}-\kappa \Psi_{\alpha\beta\gamma}[r sin\phi]U_\beta^{a-b}\uuline{W_\gamma^b}\\
\end{aligned}
\end{equation}
...
\section{Basis function for the duct-Orthogonal Series}

\begin{equation}
\begin{aligned}
P^a=\sum_{\alpha=0}^{\infty}P_{\alpha}^a(s)\psi_\alpha(s,r,\theta)\\
U^a=\sum_{\alpha=0}^{\infty}U_{\alpha}^a(s)\psi_\alpha(s,r,\theta)\\
V^a=\sum_{\alpha=0}^{\infty}V_{\alpha}^a(s)\psi_\alpha(s,r,\theta)\\
W^a=\sum_{\alpha=0}^{\infty}W_{\alpha}^a(s)\psi_\alpha(s,r,\theta)\\
\end{aligned}
\end{equation}



and can be normalized according to 
\begin{equation}
\begin{aligned}
\int_0^{2\pi}\int_0^h\psi_\alpha\psi_\beta r dr d\theta=\delta_{\alpha\beta}
\end{aligned}
...
and satify the no penetration condtion on the duct wall
\begin{equation}
\begin{aligned}
{\partial \psi_\alpha\over \partial r}|_{r=h}=0, \forall \theta
\end{aligned}
\end{equation}
...
\begin{equation}
\begin{aligned}
\psi_\alpha=C_\alpha J_p({{j_{pq}r}\over h})cos(p\phi -{\xi \pi \over 2})
\end{aligned}
\end{equation}
...
\begin{equation}
\begin{aligned}
C_\alpha=\left\{\begin{matrix}
{1\over \sqrt{\pi h^2 J_0^2(j_{0q})}}\qquad p=0\\
{1\over \sqrt{{\pi h^2\over 2} (1-{p^2\over j^2_{pq}})J_0^2(j_{0q})}}\qquad p>0\\
\end{matrix}\right.\\
\end{aligned}
\end{equation}
...
\begin{equation}
\begin{aligned}
\lambda_\alpha={j_{pq}\over h}
\end{aligned}
\end{equation}

Here, $J_p$ denotes the ordinary Bessel function of the first kind and $j_{pq}$ denotes the $q^{th}$ zero of the derivative of $J_p(r)$, noting that $j_{00}=0$ (corresponding to plane wave modes) and $j_{0q}>0 $ for $q>0$. The index $\alpha$ maps to the triplet of integers $(p,q,\xi)$ with $p\in [0,\infty)$, $q\in[0,\infty)$ and $\xi=0,1$. The basis functions can be well ordered by pairing modes differing only by the $\xi$ index(aside forom the $p=0$ modes which do not come in a pair) and ordering them by the size of their eigenvalues $\lambda_\alpha \leq \lambda_{\alpha+2}$.

\subsection{Detail introduction of Basis functions}
...
\subsection{Example of Helmoholtz's equation in a hollow cylinder}

Consider Laplace's equation in a hollow cylinder of radius $h$ with endcaps at $z=0 $and $z=a$,
...
\begin{equation}
\begin{aligned}
V(a,\psi,z)=0, V(r,\psi, 0)=0,V(r,\psi, h)=f(r, \psi)
\end{aligned}
\end{equation}

for a given potential function $f(r, \psi)$. In cylindrical coordinates,
we know that:
\begin{equation}
\begin{aligned}
\nabla^2 f=\nabla \cdot \nabla f\\
={1\over {h_1 h_2 h_3}}[{{\partial(\nabla f)_1}h_2h_3]\over \partial w_1}+{{\partial(\nabla f)_2}h_1h_3]\over \partial w_2}+{{\partial(\nabla f)_3}h_1h_2]\over \partial w_3}]\\
={1\over {h_1 h_2 h_3}}[{{\partial }\over \partial w_1}({h_2h_3\over h_1}{\partial f\over \partial w_1})+{{\partial }\over \partial w_2}({h_3h_1\over h_2}{\partial f\over \partial w_2})+{{\partial }\over \partial w_3}({h_1h_2\over h_3}{\partial f\over \partial w_3})]\\
\end{aligned}
\end{equation}

In straight cylindical coordinates, http://mathworld.wolfram.com/CylindricalCoordinates.html
\begin{equation}
h_z=1,
h_r=1,
h_\psi=r
...
\nabla^2 f\\
={1\over {h_1 h_2 h_3}}[{{\partial }\over \partial w_1}({h_2h_3\over h_1}{\partial f\over \partial w_1})+{{\partial }\over \partial w_2}({h_3h_1\over h_2}{\partial f\over \partial w_2})+{{\partial }\over \partial w_3}({h_1h_2\over h_3}{\partial f\over \partial w_3})]\\
={1\over r}{\partial \over \partial r}({r {\partial f\over \partial r}}) +{1\over r^2}{\partial ^2 f\over \partial \psi^2}+{\partial^2 f\over \partial z^2}
\end{aligned}
\end{equation}
 Laplace's equation becomes
\begin{equation}
\begin{aligned}
{1\over r}{\partial \over \partial  r}({ r{\partial V\over \partial  r}}) +{1\over  r^2}{\partial ^2 V\over \partial \psi^2}+{\partial^2 V\over \partial z^2}=0
\end{aligned}
\end{equation}

Using separation of variables, we assume a solution of the form
$V(r, \psi, z)=P(r)\Phi(\psi)Z(z)$,

Substituting this from into the PDE and dividing by V yields,
\begin{equation}
\begin{aligned}
{1\over r P}{\partial \over \partial  r}({ r{\partial P\over \partial  r}}) +{1\over  r^2\Phi}{\partial ^2 \Phi\over \partial \psi^2}+{1\over Z}{\partial^2 V\over \partial z^2}=0
\end{aligned}
\end{equation}
...
\begin{equation}
\begin{aligned}
{d^2 \Phi \over d\psi^2}=-m^2\Phi
\end{aligned}
\end{equation}
where m is an integer, $\Phi(\psi)=e^{i m \psi}$.

We also have
\begin{equation}
\begin{aligned}
{d^2 Z \over dz^2}=-\lambda_\alpha^2\Phi
\end{aligned}
\end{equation}

which has solution $e^{\lambda_\alpha z}$.
Because of the boundary condition at z=0, we take linear combinations of solutions so that $Z(z)=1/2e^{\lambda_\alpha z}-1/2e^{-\lambda_\alpha z}=sinh(\lambda_\alpha z)$

Then, we have

\begin{equation}
\begin{aligned}
{d^2 P\over dr^2}+{1\over r}{dP\over dr}+(\lambda_\alpha^2-{m^2\over r^2 })P=0
\end{aligned}
\end{equation}

This is also proved to be the $\textbf{Bessel equation}$ of the oder m, which has solutions $J_m(\lambda_\alpha r)$. 

In order to satisfy the boundary condtion at $r=h$, We then have
\begin{equation}
\begin{aligned}
V(r,\psi,z)=J_m({j_{mn}\over h}r)e^{im\psi}sinh({j_{mn}\over h}z)
\end{aligned}
\end{equation}
...
\begin{equation}
\begin{aligned}
\lambda_\alpha={j_{mn}\over h}
\end{aligned}
\end{equation}

Here, $J_m$ denotes the ordinary Bessel function of the first kind and $j_{mn}$ denotes the $n^{th}$ zero of the derivative of $J_m(r)$, noting that $j_{00}=0$ (corresponding to plane wave modes) and $j_{0n}>0 $ for $n>0$. 

To satisfy the boundary condition at $z=h$, we take a linear combination of solutions of this and seek the coefficients of the expansion:
\begin{equation}
\begin{aligned}
f(r,\psi)=\sum_{m=-\infty}^{\infty}\sum_{n=1}^{\infty} c_{mn}J_m({j_{mn}\over h}r)e^{im\psi}sinh({j_{mn}\over h}a)
\end{aligned}
\end{equation}


From the orthogonality relation for Bessel functions:
\begin{equation}
\begin{aligned}
\int_0^h r J_m ({j_{mi}\over h}r)J_m ({j_{mj}\over h}r)dr={h^2\over 2}[J_{v+1}(j_{mj})]^2\delta_{ij}
\end{aligned}
\end{equation}
...
\begin{equation}
\begin{aligned}
\int_0^{2\pi} e^{im\psi}e^{im'\psi}d\psi=2\pi \delta_{mm'}
\end{aligned}
\end{equation}
