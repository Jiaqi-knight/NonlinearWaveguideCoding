...
omega=1.0; density=1.0; t1=4/9; t2=1/9; t3=1/36; c_squ=1/3; 
% nx=64; ny=64;
nx=16; ny=16;
...
avu=1; prevavu=1; ts=0; deltaU=1e-7; numactivenodes=sum(sum(1-BOUND));%pore
while (ts<4000 & 1e-10<abs((prevavu-avu)/avu)) | ts<100
    ...
    DENSITY=sum(F,3);
    UX=(sum(F(:,:,[1 2 8]),3)-sum(F(:,:,[4 5 6]),3))./DENSITY;% x axis: [1 2 8] - [4 5 6]
    UY=(sum(F(:,:,[2 3 4]),3)-sum(F(:,:,[6 7 8]),3))./DENSITY;% y axis: [2 3 4] - [6 7 8]
    UX(1,1:ny)=UX(1,1:ny)+deltaU; %Increase inlet pressure
    UX(ON)=0; UY(ON)=0; DENSITY(ON)=0;% bound
...
% figure;colormap(gray(2));image(~BOUND');hold on;
quiver(2:nx,1:ny,UX(2:nx,:)',UY(2:nx,:)','r','linewidth',1);
