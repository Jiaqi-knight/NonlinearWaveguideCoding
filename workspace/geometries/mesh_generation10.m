...
phi = (0.75*pi:0.5*pi/(Neta1-1):1.25*pi)';
%phi = (pi:-pi/(Neta3-1):0)';
theta = 5*pi/4*ones(Neta1,1);
e1(:,1:3) = [xfunc(R,phi,theta) c1(1,2)*ones(length(phi),1) zfunc(R,phi,theta)];%-R*sin(phi(1))*cos(theta)+c1(1,1)
...
phi = (0.75*pi:0.5*pi/(Neta1-1):1.25*pi)';
%phi = (pi:-pi/(Neta3-1):0)';
theta = 3*pi/4*ones(Neta1,1);
e3(:,1:3) = [xfunc(R,phi,theta) c4(1,2)*ones(length(phi),1) zfunc(R,phi,theta)];
...
%phi = (pi:-pi/(Neta3-1):0)';
theta = 5*pi/4;
e5(:,1:3) = [xfunc(R,phi,theta) c1(1,2)*ones(length(phi),1) zfunc(R,phi,theta)];%-R*sin(phi(1))*cos(theta)+c1(1,1)
...
%phi = (pi:-pi/(Neta3-1):0)';
theta = 7*pi/4;
e6(:,1:3) = [xfunc(R,phi,theta) c2(1,2)*ones(length(phi),1) zfunc(R,phi,theta)];%-R*sin(phi(1))*cos(theta)+c2(1,1)
...
phi = (0.75*pi:-0.5*pi/(Neta3-1):0.25*pi)';
theta = pi/4;
e7(:,1:3) = [xfunc(R,phi,theta) c3(1,2)*ones(length(phi),1) zfunc(R,phi,theta)];%-R*sin(phi(1))*cos(theta)+c3(1,1)
...
phi = (0.75*pi:-0.5*pi/(Neta3-1):0.25*pi)';
theta = 3*pi/4;
e8(:,1:3) = [xfunc(R,phi,theta) c4(1,2)*ones(length(phi),1) zfunc(R,phi,theta)];%-R*sin(phi(1))*cos(theta)+c4(1,1)
...
phi = (0.25*pi:-0.5*pi/(Neta1-1):-0.25*pi)';
%phi = (pi:-pi/(Neta3-1):0)';
theta = 5*pi/4*ones(Neta1,1);
e9(:,1:3) = [xfunc(R,phi,theta) c5(1,2)*ones(length(phi),1) zfunc(R,phi,theta)];%-R*sin(phi(1))*cos(theta)+c1(1,1)
...
%phi = (pi:-pi/(Neta3-1):0)';
theta = 3*pi/4*ones(Neta1,1);
e11(:,1:3) = [xfunc(R,phi,theta) c8(1,2)*ones(length(phi),1) zfunc(R,phi,theta)];
...
phi = (0.75*pi:0.5*pi/(Neta1-1):1.25*pi)';
for i=1:Neta2
    theta = (pi+atan(e4(i,2)/e4(i,1)))*ones(Neta1,1);
    f1((i-1)*Neta1+1:i*Neta1,1:3) = [xfunc(R,phi,theta) e4(i,2)*ones(length(phi),1) zfunc(R,phi,theta)];
end
...
phi = (0.25*pi:-0.5*pi/(Neta1-1):-0.25*pi)';
for i=1:Neta2
    theta = (pi+atan(e12(i,2)/e12(i,1)))*ones(Neta1,1);
    f6((i-1)*Neta1+1:i*Neta1,1:3) = [xfunc(R,phi,theta) e12(i,2)*ones(length(phi),1) zfunc(R,phi,theta)];
end
...
deltaq = [deltaeta1;deltaeta2;deltaeta3];
...
ylabel('y')
zlabel('z')
title('Forma di grana - 1 it smoothing')
axis equal
...
fig6 = figure();
title('Computational and physical domains  - 1 it smoothing')
hold on
subplot(1,2,1);
subplot(1,2,2);
hsubfigs = get(fig6,'Children');
fcomp = hsubfigs(2);
fphys = hsubfigs(1);
subplot(fcomp);
plot3(geom(:,1),geom(:,2),geom(:,3),'b.')
