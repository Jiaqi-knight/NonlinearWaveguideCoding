...
w=repmat(w,N,1);
E2rho=rho*D.*T+rho.*u.^2;
mu=[rho rho.*u E2rho]; % define the mu values
N_mu=3;
lambda=zeros(size(mu)); % This produces a uniform
lambda(:,1)=0;% distribution.%可能根据平均密度决定好的初始值
lambda(:,2)=0.1;% distribution.%可能根据平均密度决定好的初始值
lambda(:,3)=0;% distribution.%可能根据平均密度决定好的初始值
