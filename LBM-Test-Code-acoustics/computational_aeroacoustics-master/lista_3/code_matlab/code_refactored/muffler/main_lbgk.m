...
%% 1 - Set lattice sizes
number_lines_lattice = 53+4; % cells in the y direction
number_columns_lattice = 197+4; % cells in the x direction

%% 2 - Set physical parameters (macro)
physical_sound_velocity = 340; % [m/s]
physical_density = 1.2; % [kg/m^3]
physical_dimension_max_x = 0.197; % [m]
physical_dimension_max_y = 0.053; % [m]
...
distance = 30;
growth_delta = 1;
[sigma_mat9_right Ft_right] = build_anechoic_condition(number_lines_lattice, ... 
number_columns_lattice, distance, growth_delta, 1);

% 4.1 - Setting conditions of wall
wall_points_up{1} = [   0           (30+61)     (30+61) (30+61+37) (30+61+37) (30+61+37+69)]; % set points in horizontal
wall_points_up{2} = [(53+6.3)/2 (53+6.3)/2    53      53 (53+6.3)/2  (53+6.3)/2  ]+2; % set points in vertical
conditions_wall_up = crossing3( number_lines_lattice, ...
number_columns_lattice, wall_points_up);
wall_points_down{1} = [   0           (30+61)     (30+61) (30+61+37) (30+61+37) (30+61+37+69)]; % set points in horizontal
wall_points_down{2} = [  (53-6.3)/2  (53-6.3)/2    0         0       (53-6.3)/2 (53-6.3)/2  ]+2;
conditions_wall_down = crossing3( number_lines_lattice, ...
number_columns_lattice, wall_points_down);

% 5 - Set initial disturbance
%initial_disturbance_density = lattice_average_density + 0.001;
%points_lattice{1} = [44]; % set all the points in x = 3
%points_lattice{2} = [32]; % set point along y
%points_lattice{2} = [26:31]; % set point along y
%points_lattice{1} = linspace(15, 15, length(points_lattice{2})); % set all the points in x = 3
%lattice = set_initial_disturbances(lattice, initial_disturbance_density, points_lattice);

%%  Times of simulation
round_trip = 2;
times_round_trip = 4;
time_final = round(times_round_trip*round_trip*number_columns_lattice*sqrt(3));
pressure_final_tube(1:time_final) = 0;
particule_velocity_final_tube(1:time_final) = 0;

%% Alocate arrays to put the pressures
pressures_input(1:time_final) = 0;
pressures_output(1:time_final) = 0;
%% Build a chirp source sound
times_chirp = 1:time_final/2;
frequency_lattice_max = 16e3*dimension_x_voxel/(340/lattice_sound_speed);
chirp_source_sound = chirp(times_chirp, 0, times_chirp(end)*2, frequency_lattice_max);
source_sound = 1 + chirp_source_sound*0.001;
for ta = 1 : time_final
    ...
    lattice = set_conditions_wall(lattice, conditions_wall_up);
    lattice = set_conditions_wall(lattice, conditions_wall_down);
    ...
    if ta <= time_final/2
        growth_delta = -1;
        [sigma_mat9_left Ft_left] = build_anechoic_condition(number_lines_lattice, ... 
        number_columns_lattice, distance, growth_delta, source_sound(ta));
    else
        growth_delta = -1;
        [sigma_mat9_left Ft_left] = build_anechoic_condition(number_lines_lattice, ... 
        number_columns_lattice, distance, growth_delta, 1);
