...
function [equilibria1,T] = LBMQuasiEquilibria(rho,u,methodChoice,nx,T)
...
equilibria1(2,:) = 2.*rho./3.*(2 - sqrt(1 + 3.*u.^2));
equilibria1(3,:) = rho./6.*( 3.*u - 1 + 2*sqrt(1 + 3.*u.^2));

%让我们来测试一下另外一种数值的办法，并对其进行优化加速
%Frapolli-ELBM4CompressibleFlow
% w=[1/6 2/3 1/6];
% c=[-1 0 1];
% [equilibria,T] =entropyEquilibrium(nx,1,3,rho.',w,c,u.',T);
