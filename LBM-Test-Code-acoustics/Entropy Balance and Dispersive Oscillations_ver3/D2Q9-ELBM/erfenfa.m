...
while abs(top1-bot1)>err
    x=(bot1+top1)/2;
    fx=F_eval(scheme,populations(i_x(k),i_y(k),:),popequilibriums(i_x(k),i_y(k),:),x);
    if fx==0
        bot1=x; top1=x;
    elseif F_eval(scheme,populations(i_x(k),i_y(k),:),popequilibriums(i_x(k),i_y(k),:),bot1)*fx<0
        top1=x;
    else
        bot1=x;
    end
