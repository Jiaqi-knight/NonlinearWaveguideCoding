...
LBM(D,Q,methodChoice,viscosity,nx,timeMax,MOV,ELBMEpsilon,Limiter,Norm)
...
[populations,T] = LBMQuasiEquilibria(scheme,rho,u,methodChoice,nx,T0); %Left moving,
...
%keyboard
[rho, u] = LBMLatticeParameters(populations,scheme); %Density and velocity are
max(abs(u))
% u=u+0.1;
% if time==100
%     r=1
...
[popequilibriums,T] = LBMQuasiEquilibria(scheme,rho,u,methodChoice,nx,T); %New
% quasiequilibria are found using new density and velocity values
[alpha , convergence] = LBMEntropicParameter(scheme,populations, ...
