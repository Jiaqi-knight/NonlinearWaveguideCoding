...
void GridObj::LBM_multi ( ) {
	...
	// Loop twice as refinement ratio per level is 2, keep time scale the same,Rohde-2006-step-1b
	int count = 1;
	do {
		...
		for (int j = j_low; j < j_high; j++) {
			for (int k = k_low; k < k_high; k++) {
				...
				if (LatTyp[idx_ijk] == 2) {
					...
					//only collision for level 1 grid, lower level collision in subgrid
...
// Coalesce operation -- called from coarse level Rohde-2006-setp5
