...
% build white noise
noise = wgn(total_time,1,0);
x = [31:40];
rho_delta = 0.1;
impulse_densities = (rho_l + rho_delta*noise);
impulse_velocities = ((rho_delta*cs/rho_l)*noise);
point_impulse = 1;
...
for ta = 1 : total_time
    ...
    if ta >= time_transient + 2 && ta < time_transient + 2 + 10 
        density_source = impulse_densities(ta);
        Ux_t = impulse_velocities(ta);
    else
