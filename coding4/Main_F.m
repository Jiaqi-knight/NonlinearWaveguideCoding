...
%% 
tic
X2.ab=         X(Base,Geo_b,2,'ab','1');
X2.ab_r=       X(Base,Geo_b,2,'ab','r');
X2.ab_r2 =     X(Base,Geo_b,2,'ab','r2');
X2.pr_ab_r=    X(Base,Geo_b,2,'pr_ab','r');
X2.ps_ab_r =   X(Base,Geo_b,2,'ps_ab','r');
X2.pr_ab_r2=   X(Base,Geo_b,2,'pr_ab','r2');
X2.a_pr_b_r =  X(Base,Geo_b,2,'a_pr_b','r');
X2.a_ps_b_r =  X(Base,Geo_b,2,'a_ps_b','r');


X3.abc=        X(Base,Geo_b,3,'ab','1');
X3.abc_r=      X(Base,Geo_b,3,'ab','r');
X3.abc_r2=     X(Base,Geo_b,3,'ab','r2');
X3.ps_abc_r=   X(Base,Geo_b,3,'ps_ab','r');
X3.ps_abc=     X(Base,Geo_b,3,'ps_ab','1');
X3.pr_abc_r=   X(Base,Geo_b,3,'pr_ab','r');
X3.pr_abc_r2=  X(Base,Geo_b,3,'pr_ab','r2');
toc 
tic
X3.a_pr_bc_r2= X(Base,Geo_b,3,'a_pr_b','r2');
X3.ab_pr_c_r2= X(Base,Geo_b,3,'ab_pr_c','r2');
X3.ps_abc_ps_r= bsxfun(@times,(X3.pr_abc_r2+X3.a_pr_bc_r2+X3.ab_pr_c_r2+3*X3.abc_r),...
    reshape((-Geo_b.h_diff./Geo_b.h),1,1,1,length(Geo_b.h)))+X(Base,Geo_b,3,'h#hC3J3','r');
%X3.ps_abc_ps_r2=X(Base,Geo_b,3,'ps(ab)','r');
toc
%%
