...
for kh=1:length(Geo_b.h)
    for ka=1:length(a)
        ...
        order_a(:,kh,ka)=find(real(eigenvalues_a)<0);
        Fun2_a.Vb_HH(:,:,kh,ka)=Fun2_a.Vb_H(length(Geo_b.m)*Geo_b.n+1:end,order_a(:,kh,ka),kh,ka);
        Fun2_a.Vb_inv_HH(:,:,kh,ka)=inv(Fun2_a.Vb_HH(:,:,kh,ka));
        Fun2_a.Lambda_HH(:,:,kh,ka)=diag(eigenvalues_a(order_a(:,kh,ka)));
    end
end
%b
for kh=1:length(Geo_b.h)
    for kb=1:length(b)
        [Fun2_b.Vb_H(:,:,kh,kb),Fun2_b.Lambda_H(:,:,kh,kb)]= eig(( Fun2_b.L(:,:,kh,kb))); %sqrt(-1)*
        eigenvalues_b=diag(Fun2_b.Lambda_H(:,:,kh,kb));
        order_b(:,kh,kb)=find(real(eigenvalues_b)<0);
        Fun2_b.Vb_HH(:,:,kh,kb)=Fun2_b.Vb_H(length(Geo_b.m)*Geo_b.n+1:end,order_b(:,kh,kb),kh,kb);
        Fun2_b.Vb_inv_HH(:,:,kh,kb)=inv(Fun2_b.Vb_HH(:,:,kh,kb));
        Fun2_b.Lambda_HH(:,:,kh,kb)=diag(eigenvalues_b(order_b(:,kh,kb)));
    end
end
%a_b
temp1=(NaN+NaN*sqrt(-1))*ones(size(Fun2_a_b.N(:,:,1,1,1)));
    ...
    for kb=1:length(a)
        for kb=1:length(b)
            if a(ka)-b(kb)==0
                ...
                [Fun2_a_b.Vb_H(:,:,kh,kb,ka),Fun2_a_b.Lambda_H(:,:,kh,kb,ka)]= eig((Fun2_a_b.L(:,:,kh,kb,ka))); %sqrt(-1)*
...
for kh=1:length(Geo_b.h)
    for ka=1:length(a)
        [Fun2_a.Wb_H(:,:,kh,ka),Fun2_a.Lambda1_H(:,:,kh,ka)]=eig(Fun2_a.YNG(:,:,kh,ka));
         Fun2_a.Wb_inv_H(:,:,kh,ka)=inv(Fun2_a.Wb_H(:,:,kh,ka));
    end
end
for kh=1:length(Geo_b.h)
    for kb=1:length(b)
        [Fun2_b.Wb_H(:,:,kh,kb),Fun2_b.Lambda1_H(:,:,kh,kb)]=eig(Fun2_b.YNG(:,:,kh,kb));
    ...
    for ka=1:length(a)
        for kb=1:length(b)
            if a(ka)-b(kb)==0
                ...
                [Fun2_a_b.Wb_H(:,:,kh,kb,ka),Fun2_a_b.Lambda1_H(:,:,kh,kb,ka)]=eig(Fun2_a_b.YNG(:,:,kh,kb,ka));
...
Fun3_a_b.YV_H=            permute(multiprod(Fun2_a_b.Y_H,Fun2_a_b.Vb_HH,[1,2]),[1,2,6,3,4,5]);
