...
plot(Geo_b.tau(kh)*ones(size(Lambda_H)),real(Lambda_H),'MarkerSize',0.15,'Marker','.','LineWidth',0.15,'LineStyle','none',...
    'Color',[1 0 0]);hold on
subplot(2,2,3);grid on
plot(Geo_b.tau(kh)*ones(size(Lambda_H)),imag(Lambda_H),'MarkerSize',0.15,'Marker','.','LineWidth',0.15,'LineStyle','none',...
    'Color',[1 0 0]);hold on
end
% 
