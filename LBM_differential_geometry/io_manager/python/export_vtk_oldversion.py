...
@function
def export_vtk(mesh=None,filename=None,*args,**kwargs):
    varargin = export_vtk.varargin
    nargin = export_vtk.nargin

    ## Create file
    
    vtkfilename=strcat(cat(num2str(c(1)),'_',num2str(c(2)),'_',num2str(c(3)),'_',filename,'_completemesh.vtk'))
...
# --> Structured points: Like the rectilinear grid, but the spacing between the points is equidistant; so only the origin and the spacing has to be given, not the point coordinates.
    ...
    fprintf(fid,strcat(cat('Mesh generated by Luca Di Stasio, Computational Physics of Engineering Materials, Institute for Building Materials, ETH Zuerich on ',num2str(c[3]),'-',num2str(c[2]),'-',num2str(c[1]),'\\n')))
    fprintf(fid,'ASCII\\n')
