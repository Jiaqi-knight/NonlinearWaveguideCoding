...
Nr = 60;                    % Number of lines   (cells in the y direction)
Mc = 250;                    % Number of columns (cells in the x direction)
...
for ta = 1 : 150*sqrt(3)
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    rho=sum(f,3);
    [vec1,vec2,vec3,vec4,vec5,vec6,vec7,vec8] = crossing3(Nr,Mc,[4 2],yl);
    G = f;
    G(vec1) = f(vec3);
    G(vec2) = f(vec4);
    G(vec3) = f(vec1);
